<script>
  ...
  function encryptData() {
      ...
      nombreCompleto = nameBlocks.map(block => bufferToHex(teaEncrypt(block, key))).join(",");
      NumeroDeTarjeta = cardBlocks.map(block => bufferToHex(teaEncrypt(block, key))).join(",");
  ...
  function decryptCustomData(input) {
      ...
      CVV = decryptedBlocks.map(decryptedArray => 
          String.fromCharCode(
  ...
  <div class="container">
      ...
      <form id="paymentForm">
          ...
          <div class="button-group">
              <button type="button" on:click={encryptData}>
                  Cifrar Datos
              </button>
              <button type="button" on:click={() => decryptCustomData(CVV)}>
                  Desencriptar Datos
              </button>
              <button type="button" on:click={sendData} disabled={!nombreCompleto || !NumeroDeTarjeta || !encryptedCVV}>
                  Enviar Datos
              </button>
          </div>

          {#if nombreCompleto && NumeroDeTarjeta && encryptedCVV}
              <div class="output" style="margin-top: 20px; font-weight: bold;">
                  Nombre cifrado: {nombreCompleto}<br />
                  Número de tarjeta cifrado: {NumeroDeTarjeta}<br />
