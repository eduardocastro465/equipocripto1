<script>
    ...
    function decryptCustomData(inputName, inputCard, inputCvv) {
      const key = globalKey;
  
      const nameArray = hexToBuffer(inputName);
      const cardArray = hexToBuffer(inputCard);
      const cvvArray = hexToBuffer(inputCvv);
  
      const decryptedNameArray = teaDecrypt(nameArray, key);
      const decryptedCardArray = teaDecrypt(cardArray, key);
      const decryptedCvvArray = teaDecrypt(cvvArray, key);
    ...
    <div class="container">
      ...
      <form id="paymentForm">
        ...
        <div class="form-group" style="margin-top: 20px;">
          <label for="inputNameToDecrypt">Nombre cifrado:</label>
          <input
            type="text"
            id="inputNameToDecrypt"
            placeholder="Ingrese nombre cifrado"
            bind:value={encryptedName}
          />
  
          <label for="inputCardToDecrypt">Número de tarjeta cifrado:</label>
          <input
            type="text"
            id="inputCardToDecrypt"
            placeholder="Ingrese tarjeta cifrada"
            bind:value={encryptedCard}
          />
  
          <label for="inputCvvToDecrypt">CVV cifrado:</label>
          <input
            type="text"
            id="inputCvvToDecrypt"
            placeholder="Ingrese CVV cifrado"
            bind:value={encryptedCVV}
          />
  
          <button
            type="button"
            on:click={() => decryptCustomData(encryptedName, encryptedCard, encryptedCVV)}
          ...
          {#if decryptedText}
            <div style="margin-top: 10px;">
              <strong>Texto desencriptado:</strong><br />
              Nombre: {decryptedText.name}<br />
              Número de tarjeta: {decryptedText.cardNumber}<br />
              CVV: {decryptedText.cvv}
            </div>
          {/if}
        </div>
      </form>
