<script>
  ...
  const manejarCifrado = () => {
    ...
    if (isNaN(despl) || despl < 0 || despl > 25 || mensaje.trim() === '') {
      toastRef.show({ severity: 'error', summary: 'Error', detail: 'Por favor, ingrese un mensaje y un desplazamiento válido (0-25).', life: 3000 });
      return;
    }
  ...
  const manejarDescifrado = () => {
    ...
    if (isNaN(despl) || despl < 0 || despl > 25 || mensajeCifrado.trim() === '') {
      toastRef.show({ severity: 'error', summary: 'Error', detail: 'Por favor, ingrese un mensaje cifrado y un desplazamiento válido (0-25).', life: 3000 });
      return;
    }
        ...
        <div class="input-group">
          <FloatLabel>
            <InputText
              ...
              type="number" <!-- Cambié a tipo number para mejor validación -->
            />
