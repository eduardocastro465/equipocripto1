<script>
  ...
  function strToUint32Array(str) {
    ...
    for (let i = 0; i < str.length; i += 4) {
      ...
      blocks.push([v0 >>> 0, v1 >>> 0]); // Asegúrate de que sean enteros sin signo
    }
    return blocks;
  }
  ...
  function decryptCustomData(input) {
    if (!globalKey) {
      alert("No se ha generado ninguna clave.");
      return;
    }
    const inputBlocks = input.split(",").map(hexToBuffer);
    const decryptedBlocks = inputBlocks.map(block => teaDecrypt(block, globalKey));
