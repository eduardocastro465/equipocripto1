<main>
    <div class="container">
      ...
      <!-- Sección para mostrar el mensaje cifrado -->
      <div id="encryptedMessage" style="margin-top: 20px; font-weight: bold;"></div>
  
      <!-- Input y botón para descifrar el mensaje -->
      <div class="form-group">
        <label for="encryptedInput">Ingresa el mensaje cifrado para descifrar:</label>
        <input type="text" id="encryptedInput" placeholder="Ingresa el texto cifrado aquí" />
        <button type="button" class="btn" onclick="decryptInput()">Descifrar</button>
      </div>
  
      <!-- Sección para mostrar el mensaje descifrado -->
      <div id="decryptedMessage" style="margin-top: 20px; font-weight: bold;"></div>
    </div>
  </main>
  ...
  <script>
      ...
      function decryptData(hexData) {
          const encryptedArray = hexToBuffer(hexData);
          const decrypted = teaDecrypt(encryptedArray, globalKey);
          return uint32ArrayToStr(decrypted);
      }
      ...
      function encryptAndSubmit() {
          ...
          document.getElementById('encryptedMessage').textContent = `Número de tarjeta cifrado: ${encryptedCard}, CVV cifrado: ${encryptedCVV}`;
      }
  
      function decryptInput() {
          const encryptedInput = document.getElementById('encryptedInput').value;
          const decrypted = decryptData(encryptedInput);
          document.getElementById('decryptedMessage').textContent = `Mensaje descifrado: ${decrypted}`;
      }
  </script>
