<script>
    ...
    const encrypt = () => {
        if (key.length === 16) { // Verifica que la clave tenga 128 bits (16 caracteres)
            // Cifrado usando TEA
    ...
    const decrypt = () => {
        if (key.length === 16) { // Verifica que la clave tenga 128 bits (16 caracteres)
            ...
            decryptedText = CryptoJS.TEA.decrypt(cipherText, key).toString(CryptoJS.enc.Utf8);
        } else {
            alert("La clave debe tener 16 caracteres.");
        }
    };
...
<main>
    ...
    <div>
        ...
        <input type="text" bind:value={key} placeholder="Ingresa la clave de 16 caracteres" maxlength="16" />
